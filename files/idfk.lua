...
runLunar(function()
    ...
    Notifier = GuiLibrary.ObjectsThatCanBeSaved['WorldWindow'].Api.CreateOptionsButton({
        Name = "PlayerNotifier",
        HoverText = "Notifies you when a player joins, player leaves and player spawns.",
        Function = function(callback)
            if callback then
              if Joins.Enabled == true then
                playersService.PlayerAdded:Connect(function(plr)
                       InfoNotification("PlayerNotifer", plr.Name.. " has joined.", Duration.Value)
                  end)
                end 
                if Leaves.Enabled == true then
    ...
    Duration = Notifier.CreateSlider({
        Name = "Duration",
        Min = 3,
        Max = 30,
        HoverText = "Duration of the Notifer Duration",
        Function = function() end,
        Default = 10
    })
    Joins = Notifier.CreateToggle({
        Name = "Joins",
